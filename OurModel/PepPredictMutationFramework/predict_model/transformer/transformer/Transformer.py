...
from OurModel.PepPredictMutationFramework.predict_model.transformer.trans1utils.Criterion import *
from OurModel.PepPredictMutationFramework.predict_model.transformer.transblock.Embedding import Embedding
from OurModel.PepPredictMutationFramework.predict_model.transformer.transblock.Encoder import Encoder
from OurModel.PepPredictMutationFramework.predict_model.transformer.transblock.Decoder import Decoder
import torch.nn.functional as F

class Transformer(nn.Module):
    def __init__(self,vocab_size,d_model=64,n_enc_layers=1,n_enc_heads=1,n_dec_layers=1,n_dec_heads=1,d_k=64,d_v=64,d_ff=512,hla_pep_concat_len=49,pep_tcr_concat_len=49,dropout_rate=0.1,max_len=5000,device = torch.device("cuda:0")):
    ...
    def predict(self, loader, num_group=1, threshold=0.5, n_layer_idx=0):
        ...
        self.eval()
        with torch.no_grad():
            val_y_prob_list, val_dec_concat_self_attentions_list = [], []
            for val_first_inputs, val_second_inputs in tqdm(loader):
                val_first_inputs, val_second_inputs = val_first_inputs.to(self.device), val_second_inputs.to(
                    self.device)

                # val_outputs:  [batch_size,2]
                # dec_concat_self_attentions:[n_layers,batch_size,n_heads,concat_len,concat_len]
                val_outputs, _, _, val_dec_concat_self_attentions = self.forward(val_first_inputs,
                                                                                 val_second_inputs, num_group)
                # 计算预测概率
                # [batch_size,1]
                y_prob_val = nn.Softmax(dim=1)(val_outputs)[:, 1].cpu().detach().numpy()
                # [data_size,1]
                ...
                if num_group == 1:
                    # (34,15)=(hla,peptide)
                    # val_dec_concat_self_attentions_list:[data_size,n_heads,hla_max_len,peptide_max_len]
                    val_dec_concat_self_attentions_list.extend(
                        val_dec_concat_self_attentions[n_layer_idx][:, :, :34, 34:])
                else:
                    # (15,34)=(peptide,tcr)
                    # val_dec_concat_self_attentions_list:[data_size,n_heads,peptide_max_len,tcr_max_len]
                    val_dec_concat_self_attentions_list.extend(
                        val_dec_concat_self_attentions[n_layer_idx][:, :, :15, 15:])
            # 预测标签列表
            val_y_pred_list = transfer(val_y_prob_list, threshold)
        return (
            val_y_pred_list,  # 预测标签列表:[data_size,1]
            val_y_prob_list,  # 预测概率列表:[data_size,1]
            val_dec_concat_self_attentions_list  # 自注意力张量:[data_size,n_heads,first_len,second_len]
        )
    ...
    def eval_one(self,input1,input2,num_group=1,layer_dix=-1):
        ...
        # dec_concat_self_attentions:[n_layers,batch_size,n_heads,concat_len,concat_len]
            ...
            #print(val_dec_concat_self_attentions[layer_dix][:, :, :34, 34:])
            ...
            val_outputs, pep_self_attn, _, val_dec_concat_self_attentions = self.forward(val_second_inputs, val_first_inputs,
                                                                             num_group)
            #print(val_dec_concat_self_attentions[layer_dix][:, :, 15:, :15])
