...
from predict_model.retnet.retnet1util.MultiScaleRetention import MultiScaleRetention
from predict_model.retnet.retnet1util.FeedForwardNet import FeedForwardNet
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs, self_retention = self.multi_scale_retention(enc_inputs)
        # enc_outputs: [batch_size, len, d_model]
        # enc_outputs = enc_outputs + enc_inputs
        # enc_outputs: [batch_size, len, d_model]
        enc_outputs = self.feed_forward_net(enc_outputs)
        return (
            enc_outputs,    # [batch_size, len, d_model]
            self_retention  # [batch_size, n_heads, len, len]
        )
